...
六项监测的飞书示警采用“每次检查创建独立明细表、人员逐行勾选推送”的方式。目标 Base 为 `A1Kwb4tkZaZdE2s3C2dcG49Fn2d`。季度检查创建 `季度示警明细-YYYY年N季`，月度检查创建 `月度示警明细-YYYY年MM月`；同一期间重复执行时追加 `YYYYMMDD-HHMMSS` 执行时间，保证每次执行使用独立且唯一的表名。每张新表都创建与“示警推送队列”一致的19个字段，并创建、启用只绑定该表的行级手动工作流。

平台创建明细记录时把“推送”明确设为未勾选。用户在具体明细行勾选后，新表工作流沿该行“法人主体”关联记录实时读取主表 `fld2f8VqpE`（业财）并发送通知。消息中的“填写整改信息”按钮使用触发行的 `recordLink`，因此业财填写“修改的凭证号”和“修改的会计年度”后，内容直接保存在发出该通知的新明细表记录中。发送成功后，工作流把同一行的状态更新为“已提交”、记录提交时间并自动清空勾选，不需要跨表同步。

`backend/scripts/enqueue_feishu_alert_notifications.py` 保留为固定“示警推送队列”的手工兼容工具，不再由真实检查入口自动调用。它每次实时分页读取主表 `fld5uBjB9R`（公司代码）、`fld65JDObx`（公司名称）及记录 ID，仅处理公司代码非空且唯一的法人主体；预览和显式入队命令仍可用于历史排障。
...
真实全量检测完成后，平台会把所有 `ALERT` 公司及完整示警明细写入本次执行的新表。创建后立即核对19个字段的名称、类型、法人主体关联、业财查找字段、状态选项及默认值，再批量写入记录并启用本表专属工作流；任一契约不一致都会终止，不会静默生成残缺记录。归档仅接受 `source_mode=REAL` 且满足 `Base记录数 - 空公司代码数 = 实际检测公司数` 的全量报告，使用 `--max-companies` 的局部调试绝不建表。纳税调增科目准确性检查按月进入月度明细表，不会进入季度明细表。

`backend/scripts/run_real_full_validation.py` 默认在写出真实全量网页报告后创建季度、月度执行明细表；`backend/scripts/run_real_tax_adjustment_validation.py` 更新网页报告后只创建纳税调增科目能力的月度执行明细表。两条真实任务不再向固定“示警推送队列”重复写入。仅排障时可用 `--skip-alert-workflow`（兼容旧参数名 `--skip-alert-queue`）只建表不建工作流，或用 `--skip-alert-archive` 完全跳过；正式全量任务不得使用。也可先单独预演，确认基础表名及行数后再写入：

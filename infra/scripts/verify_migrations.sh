...
grep -q "0016_release_manifests" "${OUTPUT_DIR}/migration-head.txt"
"${PYTHON}" -c 'import json, pathlib, sys; pathlib.Path(sys.argv[1]).write_text(json.dumps({"empty_database_upgrade": True, "legacy_upgrade": True, "downgrade_reupgrade": True, "migration_head": "0016_release_manifests"}, ensure_ascii=False, indent=2) + "\n")' "${OUTPUT_DIR}/migrations.json"

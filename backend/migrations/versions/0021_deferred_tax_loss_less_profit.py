"""Publish the loss-less-profit deferred-tax formula.

Revision ID: 0021_deferred_tax_loss_less_profit
Revises: 0020_income_tax_refund_accuracy
"""
...
revision: str = "0021_deferred_tax_loss_less_profit"
down_revision: str | None = "0020_income_tax_refund_accuracy"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


RULE_CODE = "QUARTERLY_V3"
RULE_VERSION = "deferred-tax-loss-less-profit-reviewed"
RULE_CHANGE_REASON = (
    "Reviewed deferred-tax formula using loss carryforward less cumulative profit."
)
RULE_APPROVER = "deferred-tax-business-rule-confirmation-2026-07-22"
...
def downgrade() -> None:
    bind = op.get_bind()
    bind.execute(sa.text("LOCK TABLE rule_version, monitoring_run IN ACCESS EXCLUSIVE MODE"))
    referenced_run = bind.execute(
        sa.text(
    ...
    if referenced_run is not None:
        raise RuntimeError(
            "DEFERRED_TAX_FORMULA_DOWNGRADE_BLOCKED: QUARTERLY_V3 is referenced by "
            f"monitoring run {referenced_run}"
        )
    bind.execute(
        sa.text(

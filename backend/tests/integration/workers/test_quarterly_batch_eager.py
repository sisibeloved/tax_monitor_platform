...
def test_production_quarterly_worker_rejects_unsigned_payload_before_data_access() -> None:
    run_company_id = uuid4()
    service = _RecordingBatchService(statuses={run_company_id: "SUCCEEDED"})
    settings = Settings(
    ...
    result = app.signature(RUN_COMPANY_TASK, args=(str(run_company_id),)).apply_async().get()

    assert service.company_calls == []
...
@dataclass
class _ScopeAwareBatchService:
    expected: dict[UUID, UUID]
    company_scopes: list[frozenset[UUID]] = field(default_factory=list)
    summary_scope: frozenset[UUID] = frozenset()
    ...
    def run_company(
        ...
        *,
        ...
        del task_id, automatic_retry_pending
        principal = current_principal()
        assert principal is not None and principal.is_service
        assert principal.service_scope is not None
        assert principal.service_scope.signature_verified is True
        assert principal.allowed_company_ids == frozenset({self.expected[run_company_id]})
        self.company_scopes.append(principal.allowed_company_ids)
        return {
            "run_company_id": str(run_company_id),
            "status": "SUCCEEDED",
            "retryable": False,
    ...
    def summarize(self, *, run_id: UUID) -> dict[str, object]:
        principal = current_principal()
        assert principal is not None and principal.is_service
        self.summary_scope = principal.allowed_company_ids
        return {"run_id": str(run_id), "status": "SUCCEEDED"}


def test_signed_quarterly_tasks_bind_the_exact_company_scope() -> None:
    run_id = uuid4()
    run_company_ids = (uuid4(), uuid4())
    company_ids = (uuid4(), uuid4())
    service = _ScopeAwareBatchService(dict(zip(run_company_ids, company_ids, strict=True)))
    settings = Settings(
        redis_url="redis://localhost:6379/15",
        environment="test",
        celery_task_always_eager=True,
        ...
        celery_task_store_eager_result=True,
        quarterly_task_max_retries=0,
        worker_scope_secret="signed-quarterly-worker-scope-test",
    )
    app = create_celery_app(settings)
    register_quarterly_tasks(app=app, service_factory=lambda: service)
    ...
    assert service.company_scopes == [
        frozenset({company_ids[0]}),
        frozenset({company_ids[1]}),
    ]
    assert service.summary_scope == frozenset(company_ids)

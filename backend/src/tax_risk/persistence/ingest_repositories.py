...
class IngestRepository:
    ...
    def list_companies(
        ...
        *,
        ...
        statement = (
            select(Company)
            .where(Company.id.in_(ordered_ids))
            .order_by(Company.id)
        )
        if for_update:
    ...
    def list_batches(
        ...
        *,
        ...
        ordered_ids = sorted(set(batch_ids))
        if not ordered_ids:
            return []
        statement = (
            select(IngestBatch)
            .where(IngestBatch.id.in_(ordered_ids))
    ...
    def list_source_records(
        ...
        *,
        ...
        statement = (
            select(SourceRecord)
            .where(SourceRecord.batch_id.in_(ordered_ids))
            .order_by(SourceRecord.batch_id, SourceRecord.source_record_key, SourceRecord.id)
        )
        if for_update:
            statement = statement.with_for_update().execution_options(populate_existing=True)
        return list(self._session.scalars(statement))
    ...
    def list_batch_errors(
        ...
        *,
        ...
        statement = (
            select(IngestError)
            .where(IngestError.batch_id.in_(ordered_ids))
            .order_by(IngestError.batch_id, IngestError.row_number, IngestError.id)
        )
        if for_update:
            statement = statement.with_for_update().execution_options(populate_existing=True)
        return list(self._session.scalars(statement))

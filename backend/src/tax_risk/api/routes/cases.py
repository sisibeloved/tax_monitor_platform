...
def list_risk_cases(
    ...
    with uow_factory() as uow:
        ...
        rows = uow.session.execute(
            select(RiskCase, Company.company_code, Company.company_name)
            .select_from(joined)
            .where(*conditions)
        ...
        items = tuple(
            ...
            for risk_case, company_code, company_name in rows
        )
...
def apply_case_action(
    ...
    with uow_factory() as uow:
        ...
        try:
            target = transition_case(from_status, CaseStatus(body.to_status.value))
